...
// filenames
{
    // Linux always(?) preserves the case of names
    return 1;
}
...
{
    // Linux never(?) ignores case in file names
    return 0;
}
...
{
    // Linux does not support drives, so a request to include the drive
    // letter is invalid
