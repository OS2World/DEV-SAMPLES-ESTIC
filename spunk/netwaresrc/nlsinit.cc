...
{
    unsigned Country = DefaultCountry;
    unsigned Language = DefaultLanguage;

    // Get country information. Here I'm probably supposed to call
    // NWLLocaleconv, but I have no docs what the pointer given as argument is
    // for, so assume we don't have such a function. This may change later...

    // Try if there is an override for the country in the environment
    Country = GetEnvNum ("SPUNK_COUNTRY", Country);

    // Try if there is an override for the language in the environment
    Language = GetEnvNum ("SPUNK_LANGUAGE", Language);
